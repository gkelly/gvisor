...
func TestMetricProfiling(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			// Wait for final metrics flush
			time.Sleep(10 * profilingRate)
			...
			expectedHeader := "Time," + strings.Join(test.metricNames, ",")
			lines.Scan()
			header := lines.Text()
			...
			expectedMinNumDatapoints := int(0.8 * float32(test.numIterations))
